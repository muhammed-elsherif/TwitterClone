...
class User{
	...
	public function checkQuestion($question){
		$stmt = $this->pdo->prepare("SELECT `securityQuestion` FROM `users` WHERE `securityQuestion` = :securityQuestion");
		$stmt->bindParam(':securityQuestion', $question, PDO::PARAM_STR);
		$stmt->execute();
			...
			if(in_array($ext, $allowed_extensions)){
				...
				if($errors ===0){
					...
					if($fileSize <= 5242880){
		 				...
		 				$root = 'users/' . $filename;
					  	move_uploaded_file($fileTmp,$_SERVER['DOCUMENT_ROOT'].'/twitter/views/'.$root);
						return $root;
    ...
    public function getDate($user_id){
		$stmt = $this->pdo->prepare('SELECT created_at FROM `users` WHERE `user_id` = :user_id');
		$stmt->bindParam(':user_id', $user_id, PDO::PARAM_INT);
		$stmt->execute();
		$result = $stmt->fetch();
